...
): EmitResult
{
  ...
  {
    const executable = stripExports(stripImports(script.content));
    const declared = extractDeclaredNames(executable);
    moduleVars.push(...declared.variables);
    moduleFuncs.push(...extractFunctionNames(executable));
  }
...
${setupEntries.join("\n")}
  ...
  const resolvedImportAssignments: string[] = [];

  component.scripts.forEach((script, scriptIndex) =>
  {
    if (script.type !== "module") return;

    const bindings: string[] = [];
    for (const parsed of parseImports(script.content))
    {
      let moduleName = importedModules.get(parsed.specifier);
      if (!moduleName)
      {
        moduleName = `__ljs_module_${importedModules.size}`;
        importedModules.set(parsed.specifier, moduleName);
        importStatements.push(
          `import * as ${moduleName} from ${JSON.stringify(parsed.specifier)};`,
        );
      }

      for (const binding of parsed.imports)
      {
        const value = binding.imported === "*"
          ? moduleName
          : `${moduleName}[${JSON.stringify(binding.imported)}]`;
        bindings.push(`${JSON.stringify(binding.local)}: ${value}`);
      }
    }

    if (bindings.length)
    {
      resolvedImportAssignments.push(
        `component.scripts[${scriptIndex}].resolvedImports = { ${bindings.join(", ")} };`,
      );
    }
  });
...
export const component = ${descriptor};
    ${resolvedImportAssignments.join("\n")}
